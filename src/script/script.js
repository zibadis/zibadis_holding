...
const plateMenus = [
  ...
  [
    ...
    { logo: "./src/images/logos/antopeloni.svg" },
  ...
  //  3-Zibadis
  [{ logo: "./src/images/logos/zibadis.svg" },
  ...
  //  4-zibadisplay
  [{ logo: "./src/images/logos/zibadisplay.svg" },
  ...
  //5-zinat.net
  [
    { logo: "./src/images/logos/zinatnet.svg" },
  ...
  //  8-yottabyte
  [{ logo: "./src/images/logos/yottabyte.svg" },
...
document.getElementById('minimize-btn').addEventListener('click', () => {
  // می‌توانید حالت Minimize را با کاهش اندازه منو پیاده‌سازی کنید
  gsap.to(mainGroup.scale, { x: 0.5, y: 0.5, z: 0.5, duration: 0.5 });
});

document.getElementById('maximize-btn').addEventListener('click', () => {
  // می‌توانید حالت Maximize را با بازگرداندن اندازه منو به حالت اصلی پیاده‌سازی کنید
  gsap.to(mainGroup.scale, { x: 1, y: 1, z: 1, duration: 0.5 });
});
...
async function updatePlateMenu(plateIndex) {
  ...
  brand.innerHTML = "";
  // دریافت منوی فعلی
  const currentMenu = plateMenus[plateIndex] ? [...plateMenus[plateIndex]] : [];

  // بررسی وجود عضو لوگو (فرض می‌کنیم اولین عضو آرایه مربوط به لوگو است)
  if (currentMenu.length && currentMenu[0].logo) {
    const logoItem = currentMenu.shift(); // حذف اولین عضو از آرایه
    const logoDiv = document.createElement("div");
    logoDiv.className = "plate-logo"; // کلاس دلخواه برای استایل دهی

    const logoImg = document.createElement("img");
    logoImg.src = logoItem.logo; // مسیر لوگو
    logoImg.alt = "Logo";

    logoDiv.appendChild(logoImg);
    // افزودن لوگو به container یا به هر المان دیگری که مد نظر دارید
    brand.appendChild(logoDiv);
  }

  // حالا پردازش بقیه آیتم‌ها به عنوان منو
  for (const item of currentMenu) {
    ...
    a.href = "#"; // غیرفعال کردن تغییر مسیر به صورت پیش‌فرض

    // اگر آیکونی هم وجود داشته باشد (مثلاً به عنوان item.icon)
    if (item.icon) {
      const iconImg = document.createElement("img");
      iconImg.classList.add("bi", "me-2");
      iconImg.setAttribute("width", "16");
      iconImg.setAttribute("height", "16");
      iconImg.src = item.icon; // استفاده از مسیر آیکون
      a.appendChild(iconImg);
    }

    // اضافه کردن متن منو به <a>
    a.appendChild(document.createTextNode(item.label));

    // event listener برای مدیریت active بودن و لود صفحه
    a.addEventListener("click", (event) => {
      event.preventDefault();
      // حذف کلاس active از تمامی لینک‌های موجود در منوی بالایی
      document.querySelectorAll("#topMenuList a").forEach(link => {
...
const layout = `
 ...
 <div class=" ">
  ...
  <div class="side-nav offcanvas offcanvas-start d-flex flex-column flex-shrink-0 p-3 bg-light" style="width: 280px;"  data-bs-backdrop="false" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
     <button class="close-btn btn btn-primary" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasExample" aria-controls="offcanvasExample">
    
      <i class="bi bi-arrow-bar-left"></i>
    </button>
    <a href="/" class="d-flex align-items-center mb-3 mb-md-0 me-md-auto link-dark text-decoration-none" >
      <div class="bi me-2" id="brand"> </div>
      <span class="fs-4">Sidebar</span>
    </a>
    <hr>
    <ul class="nav nav-pills flex-column mb-auto" id="topMenuList">
    ...
    <div class="dropdown">
      ...
      <ul class="dropdown-menu text-small shadow" aria-labelledby="dropdownUser2">
        <li><a class="dropdown-item" href="#">Regular user</a></li>
        <li><a class="dropdown-item" href="#">Office user</a></li>
        <li><a class="dropdown-item" href="#">Governance user</a></li>
  ...
  `
const parentDiv = document.getElementById("infoModal");

parentDiv.insertAdjacentHTML("afterbegin", layout);
